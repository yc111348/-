...
bool SingleConnectChooseScene::init()
{
    ...
    auto *WRM = Label::createWithTTF("选择游戏模式！","fonts/chicken.ttf", 24);
    ...
    addChild(single,2);
    auto *singlettf = Label::createWithTTF("单人模式", "fonts/chicken.ttf", 20);
    singlettf -> setPosition(size.width*2/7,size.height/3-20);
    ...
    addChild(connect,2);
    auto *connectttf = Label::createWithTTF("联机模式", "fonts/chicken.ttf",20);
    connectttf -> setPosition(size.width*6/7, size.height/3-20);
